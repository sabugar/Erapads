...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fo="http://www.w3.org/1999/XSL/Format">
	...
	<xsl:template match="/">
		<xsl:apply-templates select="runsheet"/>
	</xsl:template>

	<xsl:template match="runsheet">
		<document>
			<template leftMargin="2.0cm" rightMargin="2.0cm" topMargin="2.0cm" bottomMargin="2.0cm" title="Menifest" author="Generated by Open ERP">
				<pageTemplate id="all">
					<pageGraphics/>
					<xsl:apply-templates select="runsheet-line" mode="frames"/>
				</pageTemplate>
			</template>
			<stylesheet>
			...
			<story>
				<xsl:apply-templates select="runsheet-line" mode="story"/>
			</story>
		</document>
	</xsl:template>

	<xsl:template match="runsheet-line" mode="frames">
	...
	<xsl:template match="runsheet-line" mode="story">
	    <spacer length="0.3cm"/>
	    <barCode><xsl:value-of select="name"/></barCode>
		<para style="nospace"><xsl:value-of select="name"/><xsl:text>, </xsl:text><xsl:value-of select="date"/><xsl:text>,</xsl:text>,<xsl:value-of select="delivery_id"/></para>
		<para style="nospace">
	        <xsl:choose>
